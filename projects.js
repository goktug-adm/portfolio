...
window.PROJECTS = [
...

  {
    ...
    sections: [
      ...
      { h: "Technical Architecture", b: "<p>The platform is built as a five-stage pipeline:</p><ol><li><strong>Camera layer</strong> — ingests video from new or existing in-kitchen cameras.</li><li><strong>Data streaming</strong> — frames flow into the analysis backend.</li><li><strong>Image processing &amp; AI analysis</strong> — object detection / tracking models flag people, PPE items (hat, apron, gloves), tools and unsafe contact events (e.g. dropped food still served, no-handwash entries).</li><li><strong>Event management</strong> — detected events are deduplicated, classified by severity and routed.</li><li><strong>Notification &amp; intervention</strong> + <strong>reporting &amp; learning</strong> — managers get an instant alert; the system learns from outcomes to keep reducing false positives.</li></ol>" },
